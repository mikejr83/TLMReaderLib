...
public class HeaderRpmBlockTest {
	...
	public void testIsRpmHeaderOK() {
		byte[] testBlock = new HeaderBlockBuilder(0, RPM_HEADERBLOCK_MAKER).get();
	...
	public void testIsRpmHeaderPartlyWrongHeaderMarker() {
		byte[] testBlock = new HeaderBlockBuilder(0, 0x7E11).get();
		Assert.assertFalse(HeaderRpmBlock.isRpmHeader(testBlock));
	}
	...
	public void testGetPoles() {
		byte[] testBlock = new HeaderBlockBuilder(0, RPM_HEADERBLOCK_MAKER).setValue(0x0600, 0x06).get();
	...
	public void testIsActive() {
		byte[] testBlock = new HeaderBlockBuilder(0, RPM_HEADERBLOCK_MAKER).setValue(0x0001, 0x06).get();
		HeaderRpmBlock rpmHeaderBlock = new HeaderRpmBlock(testBlock);
	...
	public void testGetMinRpm() {
		byte[] testBlock = new HeaderBlockBuilder(0, RPM_HEADERBLOCK_MAKER).setSwitchedEndianValue((short)2300, 0x0C).get();
		HeaderRpmBlock rpmHeaderBlock = new HeaderRpmBlock(testBlock);
		assertEquals(2300, (short)rpmHeaderBlock.getMinRpm());
	}
	@Test
	public void testGetMaxRpm() {
		byte[] testBlock = new HeaderBlockBuilder(0, RPM_HEADERBLOCK_MAKER).setSwitchedEndianValue((short)6000, 0x0E).get();
		HeaderRpmBlock rpmHeaderBlock = new HeaderRpmBlock(testBlock);
		assertEquals(6000, (short)rpmHeaderBlock.getMaxRpm());
	}
	@Test
	public void testGetStatusReport5s() {
		byte[] testBlock = new HeaderBlockBuilder(0, RPM_HEADERBLOCK_MAKER).setValue(0x0100, 0x10).get();
		HeaderRpmBlock rpmHeaderBlock = new HeaderRpmBlock(testBlock);
		assertEquals("5 sec", rpmHeaderBlock.getStatusReport());
	}
	@Test
	public void testGetWarningReport10s() {
		byte[] testBlock = new HeaderBlockBuilder(0, RPM_HEADERBLOCK_MAKER).setValue(0x0002, 0x10).get();
		HeaderRpmBlock rpmHeaderBlock = new HeaderRpmBlock(testBlock);
